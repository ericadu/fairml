...
if __name__ == '__main__':
    ...
    for col in pp_data.columns.values:
        # if col in {'Number_of_Priors'}:
        #     new_row.append(50 + randint(-5, 10))
        #     new_row_black.append(0)
        if col in {'score_factor', 'Two_yr_Recidivism', 'Age_Below_TwentyFive'}:
            new_row.append(1)
            new_row_black.append(0)
        elif col in {'African_American', "Age_Above_FourtyFive"}:
    ...
    with open(out_file, 'w') as outf:
        ...
        for _ in range(int(args.epsilon * pp_data.shape[0])):
            new_row[priors_index] = 50 + randint(-5, 10)
